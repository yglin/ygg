...
/**
 * Data source for the SchedulePlanTable view. This class should
 * encapsulate all logic for fetching and manipulating the displayed data
 * (including sorting, pagination, and filtering).
 */
export class SchedulePlanTableDataSource extends DataSource<SchedulePlan> {
  ...
  constructor(dataSource: Observable<SchedulePlan[]> | SchedulePlan[]) {
    super();

    // Build the data flow
    this.updateEvent$ = new BehaviorSubject(null);
    let dataSource$: Observable<SchedulePlan[]>;
   ...
   */
  connect(): Observable<SchedulePlan[]> {
   ...
   */
  getProcessedData(data: SchedulePlan[]): SchedulePlan[] {
    let outputData: SchedulePlan[] = [...data];
  ...
  getFilteredData(data: SchedulePlan[]) {
    if (!this.filter) {
      return data;
    } else {
   ...
   */
  private getPagedData(data: SchedulePlan[]) {
   ...
   * this would be replaced by requesting the appropriate data from the server.
   */
  private getSortedData(data: SchedulePlan[]) {
