...
import { DateRangeViewPageObjectCypress, DayTimeRangeViewPageObjectCypress } from '@ygg/shared/omni-types/test';
...
export class TourPlanViewPageObjectCypress extends TourPlanViewPageObject {
  ...
  expectValue(tourPlan: TheThing) {
    ...
    const dateRangeViewPO = new DateRangeViewPageObjectCypress(this.getSelector('dateRange'));
    const dateRange: DateRange = tourPlan.cells['預計出遊日期'].value;
    dateRangeViewPO.expectValue(dateRange);

    const dayTimeRangeViewPO = new DayTimeRangeViewPageObjectCypress(this.getSelector('dayTimeRange'));
